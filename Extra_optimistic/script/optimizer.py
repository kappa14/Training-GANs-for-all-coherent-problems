...
class optimAdam(Optimizer):
    ...
    def __init__(self, lr=0.001, beta_1=0.9, beta_2=0.999,
        ...
        super(optimAdam, self).__init__(name='optimAdam', **kwargs)
        with K.name_scope(self.__class__.__name__):
            self.iterations = K.variable(0, dtype='int64', name='iterations')
            self.learning_rate = K.variable(lr, name='lr')
...
class ExtraoptimAdam(Optimizer):
    """Adam optimizer with OMD Dynamics and Extrapolation step.
    Default parameters follow those provided in the original paper.
    # Arguments
        lr: float >= 0. Learning rate.
        beta_1: float, 0 < beta < 1. Generally close to 1.
    ...
    def __init__(self, lr=0.001, beta_1=0.9, beta_2=0.999,
                 epsilon=1e-8, decay=0., **kwargs):
        super(ExtraoptimAdam, self).__init__(name='ExtraOptimAdam', **kwargs)
        with K.name_scope(self.__class__.__name__):
            self.iterations = K.variable(0, dtype='int64', name='iterations')
            self.learning_rate = K.variable(lr, name='lr')
            self.beta_1 = K.variable(beta_1, name='beta_1')
    ...
    def get_updates(self, loss, params):
        ...
        params_copy = keras.models.clone(params)
        
        for p, g, m, v, m_old, v_old in zip(params_copy, grads, ms, vs, ms_old, vs_old):
            m_t = (self.beta_1 * m) + (1. - self.beta_1) * g
            v_t = (self.beta_2 * v) + (1. - self.beta_2) * K.square(g)
            ...
            p_t = p - lr_t * m_hat / (K.sqrt(v_hat) + self.epsilon)
        ...
        grads_2 = self.get_gradients(loss, params_copy)
        
        ms_2 = [K.zeros(K.int_shape(p), dtype=K.dtype(p)) for p in params_copy]
        vs_2 = [K.zeros(K.int_shape(p), dtype=K.dtype(p)) for p in params_copy]
        ms_old_2 = [K.zeros(K.int_shape(p), dtype=K.dtype(p)) for p in params_copy]
        vs_old_2 = [K.zeros(K.int_shape(p), dtype=K.dtype(p)) for p in params_copy]
        
        for p, g, m, v, m_old, v_old in zip(params_copy, grads_2, ms_2, vs_2, ms_old_2, vs_old_2):
            m_t = (self.beta_1 * m) + (1. - self.beta_1) * g
            v_t = (self.beta_2 * v) + (1. - self.beta_2) * K.square(g)
    ...
    def get_config(self):
        config = {'lr': float(K.get_value(self.learning_rate)),
                  ...
                  'decay': float(K.get_value(self.decay)),
                  'epsilon': self.epsilon}
        base_config = super(ExtraoptimAdam, self).get_config()
        return dict(list(base_config.items()) + list(config.items()))
